<script lang="ts">
	...
	onMount(() => {
		if ($templates.length) {
			const checkIndex = $templates.findIndex((t) => t.name === decodeURI(data.slug));
			if (checkIndex < 0) goto('/');
			else {
				$selectedTemplate = checkIndex;
				$table = [...days(interval)].map((d) => ({
					date: d.toFormat('dd/MM/yyyy'),
					isPresent: true,
					note: ''
